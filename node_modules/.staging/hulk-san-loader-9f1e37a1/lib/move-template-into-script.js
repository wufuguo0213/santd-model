...
module.exports = function(__sanParts__, config) {
    ...
    let templateLiteralAst;
    try {
        templateLiteralAst = babel.parse(wrappedTemplateQuote(sanTemplateString)).program.body[0].expression;
    } catch (e) {
        templateLiteralAst = babel.parse(wrappedTemplateQuote(sanTemplateString), {filename: '_fake_'}).program.body[0]
            .expression;
    }
    ...
    try {
        ...
        sanScriptAst = babel.parse(__sanParts__.script.content, {filename: '_fake_'});
    }
